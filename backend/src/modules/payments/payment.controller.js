...
const purchase = asyncHandler(async (req, res) => {
  const transaction = await service.purchaseListing(req.user.id, req.params.datasetId);
...
const release = asyncHandler(async (req, res) => {
  const transaction = await service.releaseTransaction(req.user.id, req.params.id);
  success(res, transaction);
});

const dispute = asyncHandler(async (req, res) => {
  const transaction = await service.raiseDispute(req.user.id, req.params.id, req.body.reason);
  success(res, transaction);
});
