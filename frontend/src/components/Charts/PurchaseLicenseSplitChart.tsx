...
interface PurchaseLicenseSplitChartState {
  series: number[];
}
...
const LEGEND = [
  { label: "One-time download", value: 40, color: "bg-[#2F5F3F]" },
  { label: "Time-limited access", value: 35, color: "bg-[#D9A441]" },
  { label: "Research-only", value: 25, color: "bg-[#8FBF9F]" },
];

const PurchaseLicenseSplitChart: React.FC = () => {
  const [state] = useState<PurchaseLicenseSplitChartState>({
    series: [40, 35, 25],
  });
...
export default PurchaseLicenseSplitChart;
