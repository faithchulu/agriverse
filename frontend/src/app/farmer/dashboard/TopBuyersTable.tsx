...
// TODO: replace with `const { data } = await axios.get("/api/farmer/top-buyers")`
const topBuyers: TopBuyer[] = [
  {
    name: "GreenGrain Research Co.",
    datasetsPurchased: 3,
    totalSpent: 370,
  ...
  {
    name: "Kalundu Agri University",
    datasetsPurchased: 1,
    totalSpent: 75,
    lastPurchase: "2026-07-18",
  },
  {
    name: "Highveld Seed Traders",
    datasetsPurchased: 1,
    totalSpent: 60,
  ...
  return (
    <div className="col-span-12 rounded-lg border border-[#8FBF9F]/30 bg-white px-5 pb-2.5 pt-6 dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-8">
      ...
      <div className="flex flex-col">
        ...
        {topBuyers.map((buyer, key) => (
          <div
            className={`grid grid-cols-3 sm:grid-cols-4 ${
              key === topBuyers.length - 1
                ? ""
          ...
          >
            ...
            <div className="flex items-center justify-center p-2.5 xl:p-5">
              <p className="text-[#1B3A2B] dark:text-white">
                {buyer.datasetsPurchased}
              </p>
            </div>

            <div className="flex items-center justify-center p-2.5 xl:p-5">
              <p className="text-[#2F5F3F]">ZMW {buyer.totalSpent.toFixed(2)}</p>
            </div>
